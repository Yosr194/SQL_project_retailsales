...
3. **Average age of customer for a specific category ('Beauty')
SELECT category, ROUND(AVG(age), 2) AS AVG_AGE 
FROM retail_sales 
WHERE category LIKE 'Beauty';

4. **Total sales and total orders for each category
SELECT category, SUM(total_sale) AS total_sales, COUNT(*) AS total_orders 
FROM retail_sales 
GROUP BY category;

5. **TOP 5 customers (based on total sales)
...
6. **Number of transactions made by each gender for each category
SELECT category, gender, COUNT(gender) AS total_transactions 
FROM retail_sales 
GROUP BY category, gender;

7. **Number of orders by shift (using CTE)
WITH hourly_sale AS ( 
    SELECT *, 
        CASE 
...
8. **Number of unique customers by category
SELECT category, COUNT(DISTINCT customer_id) AS unique_customer 
FROM retail_sales 
GROUP BY category;


9. **All transactions with total_sales > 1000
SELECT * FROM retail_sales 
WHERE total_sale > 1000;

10. **Orders made in a specific date
SELECT * FROM retail_sales 
WHERE sale_date = '2022-11-05';

11. **Orders in November 2022 for a specific category ('Clothing') and quantity >= 4
SELECT * FROM `retail_sales` 
WHERE sale_date > '2022-11-01' AND sale_date < '2022-11-30' 
AND category LIKE 'Clothing' 
AND (quantity = 4 OR quantity > 4);

12. **Average sale for each month for each year (Window Function)
...
13. **Average sale for each month for each year with a specific rank (Highest Rank = 1)
SELECT sales_year, sales_month, average_sales 
FROM 
    (
    SELECT sales_year, sales_month, average_sales, 
